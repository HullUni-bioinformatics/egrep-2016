...
pdf(file="FIGURES/per_sample_rarefaction_by_tree_sp.pdf")
colors <- rainbow(5)
rar <- rarc(OAK, nrandom=100)
plot(rar$out[,6],rar$out[,1], col=colors[1], type = 'b', pch=18, ylim = range(c(rar$out[,1],rar$out[,1])), xlab='Number of samples', ylab = 'OTU richness')
...
#across all samples
pdf(file="FIGURES/per_sample_rarefaction_all.pdf")
rar <- rarc(table, nrandom=100)
plot(rar$out[,6],rar$out[,1], col='darkgreen', type = 'b', pch=18, ylim = range(c(rar$out[,2],rar$out[,3])), xlab='Number of samples', ylab = 'OTU richness')
points(rar$out[,6],rar$out[,2], col='darkgreen', type = 'l', lty=2, lwd=0.5)
points(rar$out[,6],rar$out[,3], col='darkgreen', type = 'l',lty=2, lwd=0.5)
dev.off()
